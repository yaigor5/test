...
1. Имя используемой БД - '**test**' (инициализация в **init.pl**)
2. Поле 'str' сделано типом 'text' иначе бы информация не умещалась (по условию это поле аналог varchar(255) раз не указана длина), но по логике обычно такие поля не менее чем 'text'
3. Вопрос по нормализации данных не обговорен в задании
4. Во 2 задании не совсем понятно как должна участвовать в запросе таблица 'message', т.к. в структуре не предусмотрено поле 'address'. Искать по 'str', забыто поле адреса в этой таблице (адрес отправителя нигде не использовался)...
...
Вывод результатов без превышения ограничений (тост с автоскрытием)

![info](img/2023-05-31_164219.png)

Вывод результатов с превышением ограничений (тост постоянный, с ручным скрытием)

![warning](img/2023-05-31_164322.png)

Вывод результатов с превышением ограничений в режиме отладки (флаг 'debug' секции [html] в конфиге)

![debugwarning](img/2023-05-31_164817.png)