...
3. Во 2 части не понятно что именно требуется получить, так как описан способ, но он не четкий. От уточнений вероятно изменится метод извлечения данных (через временную таблицу или иной).
